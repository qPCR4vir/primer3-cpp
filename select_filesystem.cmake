...
option(P3_CMAKE_STD_FILESYSTEM_FORCE "Use of STD filesystem?(a compilation error will ocurre if not available)" OFF)
option(P3_CMAKE_BOOST_FILESYSTEM_FORCE "Force use of Boost filesystem if available (over STD)?" ON)
option(P3_CMAKE_FIND_BOOST_FILESYSTEM "Search: Is Boost filesystem available?" ON)

if(P3_CMAKE_STD_FILESYSTEM_FORCE)
    target_compile_definitions(thal PUBLIC STD_FILESYSTEM_FORCE)
    target_link_libraries     (thal PUBLIC stdc++fs)

elseif(P3_CMAKE_BOOST_FILESYSTEM_FORCE)
    target_compile_definitions(thal PUBLIC BOOST_FILESYSTEM_FORCE)
    ...
    if(Boost_FOUND)
        target_compile_definitions(thal PUBLIC BOOST_FILESYSTEM_AVAILABLE)
            # SYSTEM - ignore warnings from here
        target_include_directories(thal SYSTEM PUBLIC "${Boost_INCLUDE_DIR}")    # ?? SYSTEM
        target_link_libraries     (thal PUBLIC ${Boost_LIBRARIES})
